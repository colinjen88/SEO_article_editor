...
### 變更 (Changed)
- 🔄 **預設 Footer 優化**
  - 移除程式內 fallback 的舊版 footer 文字（包含「宏觀風險」等過時內容）
  - 改用標準化的官方宣告文字，確保即使外部模板遺失也能顯示正確版權資訊
- 🛡️ **SEO 設定通用化**
  - 將預設的作者與組織名稱從特定品牌改為「作者名稱（預設）」與「組織名稱（預設）」
  - 避免新使用者誤用舊有範例資料，並提升專案的通用性

### 修正 (Fixed)
- 🐛修正了在沒有設定檔時，預設值會顯示為特定廠商名稱的問題

## [2.12] - 2025-12-26

### 變更 (Changed)
- 🔄 **HTML 輸出格式調整**
  - 移除匯出檔案與預覽內容的 `<html>`, `<head>`, `<body>` 外層標籤
  - 輸出內容僅保留 Schema、Style、Body 與 Footer，便於直接整合至其他編輯器

### 修正 (Fixed)
- 🐛 **瀏覽器預覽**：修正瀏覽器預覽時缺少 Footer 的問題
...
- 🔼 **SEO 資訊區塊收合功能**
  - 新增「收合/展開 SEO 資訊」按鈕，可一鍵隱藏頂部欄位，最大化內容編輯空間。
- 🖼️ **主/副圖區重構**
  - 將圖片欄位重構為「主圖區」與「副圖區 (FAQ前)」。
  - 副圖區設有啟用開關，勾選後才顯示編輯欄位並插入 HTML。
  - 徹底移除「圖片標題 (Title)」欄位，簡化 SEO 設定。
- 📏 **H1 & Caption 字數即時驗證**
  - H1 欄位新增 17 字限制提醒與計數器 `(X/17)`。
  - 圖片敘述 (Caption) 新增 46 字限制提醒與計數器 `(X/46)`。
  - 超過字數限制時，文字與計數標籤會自動變為**紅色**提示。

### 改進 (Improved)

- 🎨 **介面黃金比例調整**：編輯區塊比例調整為 3:1，提供更寬廣的文字修改視野。
- 🔄 **自動載入網路設定**：程式啟動時會自動載入上次使用的網站設定檔（靜默模式），無需手動重複載入。
- 💾 **資料持久化增強**：更新 JSON 儲存格式，完整支援副圖狀態與主/副圖欄位。
- 🩹 **介面一致性修復**：修正所有輸入框在不同主題下的白底黑字顯示問題。
...
- 🔄 **作者型別切換優化**
  - 切換 Person/Organization 時動態顯示/隱藏擴充欄位
  - Organization 模式維持原有簡潔作者資訊

### 改進 (Improved)

- 📝 **資料持久化**：作者擴充欄位自動儲存至 JSON 並可正確還原
- 🎨 **UI 互動**：作者欄位行根據選擇的型別智能展開/收合
...
- ✨ **內容區塊 HTML 模式**
  - H2 段落新增「HTML 模式」勾選框（預設勾選，保留既有行為）
  - H3 子區塊新增「HTML」勾選框（預設關閉，勾選後支援 HTML 標籤）
  - 啟用 HTML 模式後，編輯區即時支援語法高亮

- ⚙️ **網站設定載入優化**
  - 工具列右上角新增網站設定檔案選擇器
  - 顯示當前選擇的設定檔路徑
  - 自動記憶上次使用的設定檔路徑（儲存於 `config/local_settings.json`）
  - 簡化載入流程，無需每次重新瀏覽檔案

### 修正 (Fixed)
- 🐛 修正 H3 區塊預設無法使用 HTML 內容的問題
- 🎨 優化工具列介面排版，整合設定檔載入功能
...
### 新增 (Added)
- ✨ **圖片功能增強**
  - 新增圖片 alt 欄位，支援完整的圖片 SEO 設定
  - 圖片路徑拆分為前綴和檔名，提升管理便利性
  - 圖片自動置於 h1 之前，符合最佳 SEO 實踐
- 🔍 **內容驗證系統**
  - HTML/純文字欄位自動驗證功能
  - 錯誤時紅框提示，提升內容品質
  - 支援前言、段落、FAQ 等多種欄位驗證
...
- 🎨 **UI 佈局調整**: 重新排列圖片相關欄位順序
- 📝 **檔案格式擴充**: 支援多種分隔符號解析設定檔

### 修正 (Fixed)
- 🛠 **圖片處理邏輯**: 修正圖片 URL 組合與輸出邏輯
- 📋 **欄位驗證**: 改善內容類型檢測準確性
...
### 新增 (Added)
- ✨ **文章編號自動管理系統**
  - 儲存編輯檔時自動將文章編號 +1 並寫入 `number.txt`
  - 程式啟動時自動讀取最新文章編號 +1
  - 新增「新建檔案」按鈕，自動取得下一個文章編號
  - 包含未儲存變更提醒功能

### 變更 (Changed)
- 🔄 **工作流程優化**: 簡化文章編號管理流程，提升編輯效率
- 🎨 **工具列更新**: 新增「新建檔案」按鈕，重新排列工具列順序

### 修正 (Fixed)
- 🛠 **文章編號邏輯**: 修正文章編號遞增的錯誤處理機制
- 📝 **檔案管理**: 改善新建檔案時的欄位重置邏輯
...
- 🖼️ **圖片設定**: 新增圖片路徑、寬度 (預設 100%) 與高度 (預設 auto) 設定
...
- 📝 **版本資訊更新**
  - 版本號：v1.8 → v2.0
  - 底部版權資訊：「**© 2025 SEO Article Editor v2.11**  
Produced by Colinjen (colinjen88@gmail.com):colinjen88@gmail.com
- 🔔 **啟動提示優化**
